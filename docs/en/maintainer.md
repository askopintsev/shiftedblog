...
| `failed to bind host port 0.0.0.0:80` / port 80 in use during `compose up` | Old nginx container or host web server — run `./scripts/vps-prepare-for-ci.sh` on the VPS; CI deploy uses rolling update (nginx stays up when already running) |
