...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--method", type=str, default="pg_dump", choices=["pg_dump"]
        )
        parser.add_argument(
            "--skip-media",
            action="store_true",
            help="Backup only the database, skip media files",
        )
        parser.add_argument(
            "--skip-db",
            action="store_true",
            help="Backup only media files, skip the database",
        )
    ...
    def handle(self, *args, **options):
        ...
        if not skip_db:
            if not db_name:
                self.stdout.write(
                    self.style.ERROR("DB_NAME environment variable is required")
                )
                return
            if not db_user:
                self.stdout.write(
                    self.style.ERROR("DB_USER environment variable is required")
                )
                return
            if not db_password:
                self.stdout.write(
                    self.style.ERROR("DB_PASS environment variable is required")
                )
                return
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f"Failed to create backup directory {backup_dir}: {e}")
            )
            return
        ...
        if not skip_media:
            ...
            if media_root and os.path.isdir(media_root):
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"Media root not found or not a directory ({media_root}), "
        ...
        if yandex_token and backup_files:
            try:
                ...
                for path in backup_files:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"Uploaded to Yandex Disk: {remote_path}")
                    )
            ...
            self.stdout.write(
                self.style.WARNING("Nothing to upload (no backups created).")
            )
        elif not yandex_token and backup_files:
            self.stdout.write(
                self.style.WARNING("Yandex token not set—skipping upload.")
            )
