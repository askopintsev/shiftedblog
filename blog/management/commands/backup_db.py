        ...
        if not skip_media:
            ...
            if media_root and os.path.isdir(media_root):
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Media backup completed: {media_archive}"
                        )
                    )
                    backup_files.append(media_archive)
                except subprocess.CalledProcessError as e:
                    self.stdout.write(
                        self.style.ERROR(f"Media backup failed: {e}")
                    )
                    raise
            else:
                self.stdout.write(
        ...
        if yandex_token and backup_files:
            try:
                ...
                for path in backup_files:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Uploaded to Yandex Disk: {remote_path}"
                        )
                    )
            except yadisk.exceptions.YaDiskError as e:
                self.stdout.write(
                    self.style.ERROR(f"Yandex upload failed: {e}")
                )
                self.stdout.write(
                    self.style.WARNING(
            ...
            self.stdout.write(
                self.style.WARNING(
                    "Nothing to upload (no backups created)."
                )
            )
        elif not yandex_token and backup_files:
            self.stdout.write(
                self.style.WARNING(
                    "Yandex token not set—skipping upload."
                )
            )
